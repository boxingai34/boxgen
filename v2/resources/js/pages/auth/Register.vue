...
<template>
    ...
    <AuthLayout judul="Sign up" ket="New accounts need an admin's approval before they can be used.">
        <form class="space-y-4" @submit.prevent="kirim">
            ...
            <div>
                <label for="full_name" class="mb-1.5 block text-xs font-medium text-muted-foreground">
                    Name <span class="text-muted-foreground/60">(optional)</span>
                </label>
            ...
            <div>
                <label for="email" class="mb-1.5 block text-xs font-medium text-muted-foreground">
                    Email <span class="text-muted-foreground/60">(optional)</span>
                </label>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label for="password_confirmation" class="mb-1.5 block text-xs font-medium text-muted-foreground">Confirm</label>
                    <input
            ...
            <Tombol tipe="submit" ukuran="besar" penuh :nonaktif="form.processing">
                ...
                {{ form.processing ? 'Sending…' : 'Sign up' }}
            </Tombol>
        </form>

        <p class="mt-6 text-center text-sm text-muted-foreground">
            Already have an account?
            <Link :href="route('login')" class="font-medium text-[hsl(var(--sorot))] hover:underline">Sign in</Link>
        </p>
    </AuthLayout>
</template>
