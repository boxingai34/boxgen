...
async function pilihBerkas(e: Event) {
    ...
    try {
        ...
        galat.value = err instanceof GalatKirim ? err.message : 'Upload failed.';
    } finally {
        sibuk.value = false;
    ...
    <div>
        ...
        <div class="flex items-start gap-3">
            <div class="relative h-20 w-28 shrink-0 overflow-hidden rounded-lg border border-border bg-muted/40">
                ...
                <span v-else class="grid h-full w-full place-items-center text-xs text-muted-foreground">empty</span>
                <button
                    v-if="src"
                    type="button"
            ...
            <div class="min-w-0 flex-1 space-y-2">
                <input v-model="src" type="text" placeholder="/uploads/landing/... or /img/..." class="h-9 w-full rounded-lg border border-input bg-background px-3 text-xs outline-none focus:border-[hsl(var(--sorot))]" />
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    >
                        {{ pilihTerbuka ? 'Close' : `Pick from uploads (${unggahan.length})` }}
                    </button>
                </div>
