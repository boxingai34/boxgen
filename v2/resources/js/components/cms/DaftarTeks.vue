    ...
    <div>
        ...
        <ul v-if="daftar.length" class="mb-2 space-y-1.5">
            <li v-for="(t, i) in daftar" :key="i" class="flex items-center gap-1.5">
                ...
                />
                <button type="button" class="grid h-8 w-8 place-items-center rounded-lg border border-border text-muted-foreground hover:text-foreground disabled:opacity-30" :disabled="i === 0" title="Naik" @click="geser(i, -1)">
                    <ArrowUp class="h-3.5 w-3.5" />
                </button>
                <button type="button" class="grid h-8 w-8 place-items-center rounded-lg border border-border text-muted-foreground hover:text-foreground disabled:opacity-30" :disabled="i === daftar.length - 1" title="Turun" @click="geser(i, 1)">
                    <ArrowDown class="h-3.5 w-3.5" />
                </button>
                <button type="button" class="grid h-8 w-8 place-items-center rounded-lg border border-border text-muted-foreground hover:border-destructive/50 hover:text-destructive" title="Hapus" @click="hapus(i)">
                    <X class="h-3.5 w-3.5" />
                </button>
            </li>
        </ul>
        ...
        <form class="flex gap-1.5" @submit.prevent="tambah">
            <input
                v-model="baru"
                type="text"
                :placeholder="placeholder || 'Tambah…'"
